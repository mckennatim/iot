...
void Reqs::pubState(int hc){
  ...
  if((hc & 4) == 4){
    sprintf(astr, "{\"id\":%d, \"darr\":[%d, %d, %d, %d], \"new\":%d}", 2, sr.temp3.temp, sr.temp3.state, sr.temp3.hilimit, sr.temp3.lolimit, sr.temp3.rec);
    clpub(srstate, astr);
    sr.temp3.rec=0;
  }
  if((hc & 8) == 8){
    sprintf(astr, "{\"id\":%d, \"darr\":[%d, %d, %d, %d], \"new\":%d}", 3, sr.temp4.temp, sr.temp4.state, sr.temp4.hilimit, sr.temp4.lolimit, sr.temp4.rec);
    clpub(srstate, astr);
    sr.temp4.rec=0;
  }
  if((hc & 16) == 16){
...
bool Reqs::deseriCmd(){
  ...
  switch(id){
    ...
    case 1:
      ...
      break;
    case 2:
      copyHiLoState(id, sr.temp3, sra, po.temp3);          
      break;
    case 3:
      copyHiLoState(id, sr.temp4, sra, po.temp4);          
      break;
    case 4:
